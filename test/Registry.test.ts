...
describe('Registry', function () {
  ...
  beforeEach(async () => {
    ...
    const PubicListVerifierFactory =
      await ethers.getContractFactory('PublicListVerifier');
  ...
  describe('Registering domains', function () {
    ...
    it('Should register a domain with wildcard successfully', async function () {
      ...
      await registry
        .connect(domainOwner)
        .registerDomain(ALWAYS_TRUE_AUTHORIZER_ID, DOMAIN, true);
    ...
    it("Shouldn't register a domain if the authorizer returns false", async function () {
      ...
      expect(
        registry
          ...
          .registerDomain(ALWAYS_FALSE_AUTHORIZER_ID, DOMAIN, true),
      )
        .revertedWithCustomError(
          registry,
          'AccountIsNotAuthorizeToRegisterDomain',
        )
  ...
  describe('Domain ttl', function () {
    ...
    describe('Domain Owner', function () {
      ...
      it('Should return false if it is not the domain owner', async function () {
        expect(await registry.isDomainOwner(DOMAIN, notDomainOwner.address)).to
          .false;
      });

      it('Should return true if it is the domain owner', async function () {
        expect(await registry.isDomainOwner(DOMAIN, domainOwner.address)).to
          .true;
      });

      it('Should return the domain owner if it is a valid domain', async function () {
        expect(await registry.domainOwner(DOMAIN)).to.equal(
          domainOwner.address,
        );
      });

      it('Should return the zero address if it was not register', async function () {
        expect(await registry.domainOwner('another.domain.com')).to.equal(
          ZERO_ADDRESS,
        );
      });
  ...
  describe('Add Verifier', function () {
    ...
    it('Should only let the owner of the domain add a verifier', async function () {
      await registry
        .connect(domainOwner)
        .addVerifier(DOMAIN, publicListverifier.target);
      expect(await registry.domainVerifier(DOMAIN)).to.equal(
