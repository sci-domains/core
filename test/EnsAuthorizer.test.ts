...
import { ENS, ENSAuthorizer } from '../typechain-types';
...
describe('EnsAuthorizer', function () {
  let ensRegistry: ENS, ensAuthorizer: ENSAuthorizer, rootDomain: string;

  beforeEach(async () => {
    // Ens Authorization Contract Deployment
    const EnsAuthorizer = await ethers.getContractFactory('ENSAuthorizer');
    ensAuthorizer = await EnsAuthorizer.deploy(
