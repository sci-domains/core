...
import { SuperChainTargetRegistrar, SciRegistry, MockCrossDomainMessenger } from '../../types';
...
describe('SuperChainTargetRegistrar', function () {
  ...
  beforeEach(async () => {
    ...
    ({ superChainTargetRegistrar, sciRegistry } = await (ignition.deploy(
      SuperChainTargetRegistrarModule,
      {
        parameters: {
    ...
    await superChainTargetRegistrar.grantRole(
      await superChainTargetRegistrar.REGISTER_DOMAIN_ROLE(),
  ...
  describe('Register domains', function () {
    it('It should register a domain with verifier if it is called by the crossDomainMessanger and the xDomainMessageSender has the REGISTER_DOMAIN_ROLE role', async function () {
      ...
      await mockCrossDomainMessenger.sendMessage(
        superChainTargetRegistrar.target,
        superChainTargetRegistrar.interface.encodeFunctionData('registerDomainWithVerifier', [
          domainOwner.address,
          DOMAIN_HASH,
          verifier,
        ]),
    ...
    it('It should register a domain if it has the REGISTER_DOMAIN_ROLE role', async function () {
      await mockCrossDomainMessenger.sendMessage(
        superChainTargetRegistrar.target,
        superChainTargetRegistrar.interface.encodeFunctionData('registerDomain', [
          domainOwner.address,
          DOMAIN_HASH,
        ]),
    ...
    it("It should reveret with AccessControlUnauthorizedAccount if it doesn't have the REGISTER_DOMAIN_ROLE role", async function () {
      await superChainTargetRegistrar.revokeRole(
        await superChainTargetRegistrar.REGISTER_DOMAIN_ROLE(),
        xDomainMessageSender.address,
      );

      await expect(
        mockCrossDomainMessenger.sendMessage(
          superChainTargetRegistrar.target,
          superChainTargetRegistrar.interface.encodeFunctionData('registerDomain', [
            domainOwner.address,
            DOMAIN_HASH,
    ...
    it('It should reveret with InvalidMessageSender if the message sender is not the CrossDomainMessanger', async function () {
      await expect(
        superChainTargetRegistrar.connect(user).registerDomain(domainOwner.address, DOMAIN_HASH),
      )
      ...
      await expect(
        superChainTargetRegistrar
          .connect(user)
          .registerDomainWithVerifier(domainOwner.address, DOMAIN_HASH, ethers.ZeroAddress),
      )
        .to.revertedWithCustomError(superChainTargetRegistrar, 'InvalidMessageSender')
        .withArgs(user.address);
    });
