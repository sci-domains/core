...
import {
  ...
  SciRegistrar__factory,
  SciRegistry__factory,
} from '../../../types';
...
async function main() {
  ...
  const publicListVerifier = PublicListVerifier__factory.connect(
    deployedAddresses['PublicListVerifier#PublicListVerifier'],
    owner,
  );
  const sciRegistrar = SciRegistrar__factory.connect(
    deployedAddresses['SciRegstrar#SciRegistrar'],
    owner,
  );
  const sciRegistry = SciRegistry__factory.connect(
    deployedAddresses['SciRegistry#SciRegistry'],
    owner,
  );
